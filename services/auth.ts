...
export const sendOTP = async (
  ...
  try {
    ...
    const response = await fetch(
      "https://us-central1-dular5.cloudfunctions.net/api/api/user/sendotp",
      {
        method: "POST",
        headers: {
...
const verifyOTPInternal = async (
  ...
  try {
    ...
    try {
      ...
      console.warn(
        "⚠️ Firestore operation failed, but login succeeded:",
        firestoreError,
      );
      ...
      (async () => {
        try {
          ...
          if (pushToken) {
            ...
            console.warn(
              "⚠️ Could not get push token - notifications may not work",
            );
          }
        } catch (notifError) {
    ...
    if (error.code === "auth/email-already-in-use") {
      return {
        success: false,
        error:
          "This phone number is already registered. Please try signing in.",
      };
    }
