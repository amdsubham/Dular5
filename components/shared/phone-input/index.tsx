...
export const PhoneInput = ({
  ...
  const handlePhoneChange = (value: string) => {
    ...
    const isValid = validatePhoneNumber(cleanNumber);
    console.log("Phone validation:", { cleanNumber, isValid, length: cleanNumber.length });
    if (cleanNumber.length === 10 && isValid) {
      const fullPhone = `${selectedCode}${cleanNumber}`;
      console.log("Valid phone number:", fullPhone);
      onPhoneChange(fullPhone);
    } else {
      onPhoneChange(""); // Clear phone if not valid
    }
  };
  ...
  return (
    <FormControl isInvalid={!!error}>
      ...
      {error && (
        <FormControlError>
          <FormControlErrorText className="text-white">{error}</FormControlErrorText>
        </FormControlError>
      )}
    </FormControl>
  );
