...
const bottomSheetContentStyle = tva({
  base: "flex-1 items-center  bg-background-0",
});
...
const bottomSheetItemStyle = tva({
  base: "w-full flex-row items-center rounded-sm data-[disabled=true]:opacity-40 data-[disabled=true]:web:pointer-events-auto data-[disabled=true]:web:cursor-not-allowed hover:bg-background-50 active:bg-background-100 data-[focus=true]:bg-background-100 web:data-[focus-visible=true]:bg-background-100 web:data-[focus-visible=true]:outline-indicator-primary gap-2",
});
...
const BottomSheetDragIndicator = ({
  ...
  return (
    <BottomSheetHandle
      {...props}
      // @ts-ignore
      className={"items-center py-1 w-full rounded-t-3xl " + className}
    >
      {children}
    </BottomSheetHandle>
  );
