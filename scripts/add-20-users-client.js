 ...
 *
 * Run with: node scripts/add-20-users-client.js
 */
...
async function createUser(index, gender) {
  ...
  const email = `${firstName.toLowerCase()}.${lastName.toLowerCase()}${index}${timestamp}@testuser.com`;
...
async function addUsers() {
  try {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      await new Promise(resolve => setTimeout(resolve, 2000)); // Wait 2 seconds between users
